...
function On_TextArea_PreviewClick(oForm, idTextArea, idIFrame, oSender)
{
	if(oSender.checked)
	{
		...
		document.frames(idIFrame).document.bgColor = "#C0C0C0";
		document.frames(idIFrame).document.body.innerHTML = oForm.elements[idTextArea].value;
		ifPreviewed = true;
	}
	else
	{
...
//响应"相关报道检索"button
//----------------------------------------
function checkKeywordSerachSubmit(form, obj)
{
	if(event.keyCode == 13)
	{
		obj.click();
...
function On_DocumentForm_KeywordSearch(oForm, oSender, cgi, target, keyset_field)
{
	...
	var agent = "/dpa/main.php?do=www_search_agent_keyword";
	do_Rel_Result(agent, cgi, oForm, target, title, false);
}


//----------------------------------------
//处理关键字搜索,调用main.php?do=www_agent
//----------------------------------------
function do_Rel_Result(agent,cgi,form,target,title,pause)
{
	...
	else
	{
		...
		mode_value = UrlEncode(mode_value);
	}

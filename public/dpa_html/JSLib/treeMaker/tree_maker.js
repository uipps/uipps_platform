/************************************************************************************
*Title   :   treeMaker 脚本文件 (Ver 3.0.1)
*program :   区钊贤
...
function Tree_on_action(action)//fire when click node
{
	if(action.type==Tree_LINK)//link
	{
		...
		{
			/*
			var command = target +".location = "+
				(document.location.protocol=="file:"?
					"action.link;":"escape(action.link).replace(/.html%23/g,'.html#')");
			*/
...
function Tree_clickNode(nodeID)
{
	...
	//$(".Tree_FILE_1").css('font-weight','normal');
	//$("#Tree_td_"+nodeID).css('font-weight','bold');

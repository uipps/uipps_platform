...
<body topmargin="0" leftmargin="0" bgcolor="#f2faff">
<table bgcolor=#306e9f border="0" width="100%" cellspacing="2" cellpadding="3">
  <tr class="title hd">
    <td width="50%" nowrap align=left>{{$tpl_dangqianweizhi}}:{{$parent_nav}}/{{$html_name}}</td>
    <td width="50%" nowrap align=left>{{$tpl_xitongtongzhi}}:{{$l_tongzhi}}</td>
  </tr>
</table>
<hr size="1">
...
<form method="post" action="main.php" name="myform">
  {{$parent_ids_input_hidden}}
  <input type="hidden" name="do" value="{{$do}}" />
  <input type="hidden" name="action" value="{{$action}}" />
  <input type="hidden" name="type_name" value="{{$type_name}}" />
  ...
  <table width="98%" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr>
      <td>
        <input onClick="location.replace('main.php?do={{$type_name}}_add&type_name={{$type_name}}&action=add{{$parent_ids_url_build_query}}')" type=button value="{{$tpl_zengjia}}" />
        <input type=button onClick="action_onclick('main.php?do={{$type_name}}_edit&type_name={{$type_name}}&action=edit{{$parent_ids_url_build_query}}',self.document.myform,'id','edit');return false" value="{{$tpl_xiugai}}" />
        <input type=button onClick="action_onclick('main.php?do={{$type_name}}_edit&type_name={{$type_name}}&action=edit{{$parent_ids_url_build_query}}',self.document.myform,'id','del');return false" value="{{$tpl_shanchu}}" />
        {{$INPUT_other}}
      </td>
      <td>
         <div class="setPages">{{$tpl_meiyexianshi}}:<input type="text" name="pagesize_form" size="6" />{{$tpl_tiao}}<input type="submit" value="{{$tpl_shezhi}}" /></div>
      </td>
    </tr>
  </table>
  <hr size="1">
  <table bgColor=#f3f3ee border=0 cellPadding=3 cellSpacing=0 width="100%">
    <tr class="title">
      {{$show_title}}
    </tr>
    {{$show}}
